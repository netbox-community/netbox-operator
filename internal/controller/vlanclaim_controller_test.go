...
var _ = Describe("VLANClaim Controller", func() {
	...
	Context("When creating a VLANClaim", func() {
		It("Should create a Vlan resource and update its status", func() {
			...
			ipamMockVlanClaim.EXPECT().IpamVlansList(gomock.Any(), gomock.Any()).Return(mockVlansListResponse(), nil).AnyTimes()
			ipamMockVlanClaim.EXPECT().IpamVlansList(gomock.Any(), gomock.Any(), gomock.Any()).Return(mockVlansListResponse(), nil).AnyTimes()
