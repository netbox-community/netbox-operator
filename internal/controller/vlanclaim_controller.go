...
func (r *VLANClaimReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		...
		vlanResource := generateVlanFromVlanClaim(o, vlanModel.VlanId, logger)
		if err = controllerutil.SetControllerReference(o, vlanResource, r.Scheme); err != nil {
			return ctrl.Result{}, err
		}

		if err = r.Client.Create(ctx, vlanResource); err != nil {
			if errReport := r.EventStatusRecorder.Report(ctx, o, netboxv1.ConditionVlanAssignedFalse, corev1.EventTypeWarning, err); errReport != nil {
				return ctrl.Result{}, errReport
			}
			return ctrl.Result{}, err
		...
		updatedVlanSpec := generateVlanSpec(o, vlan.Spec.VlanId, logger)
...
func generateVlanFromVlanClaim(o *netboxv1.VLANClaim, vid int, logger any) *netboxv1.Vlan {
	return &netboxv1.Vlan{
		ObjectMeta: metav1.ObjectMeta{
			Name:      o.Name,
			Namespace: o.Namespace,
		},
		Spec: generateVlanSpec(o, vid, logger),
	}
}

func generateVlanSpec(o *netboxv1.VLANClaim, vid int, logger any) netboxv1.VlanSpec {
	name := o.Spec.Name
	if name == "" {
		name = o.Name
	}
