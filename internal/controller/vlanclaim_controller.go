...
func (r *VLANClaimReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if apismeta.FindStatusCondition(o.Status.Conditions, "Ready") == nil {
		r.EventStatusRecorder.Report(ctx, o, netboxv1.ConditionVlanClaimReadyFalse, corev1.EventTypeNormal, nil)
	}
	...
	if err != nil {
		...
		if !locked {
			r.EventStatusRecorder.Recorder().Eventf(o, corev1.EventTypeWarning, "FailedToLockVlanGroup", "failed to lock vlan group %s", o.Spec.VlanGroup)
		...
		if err != nil {
			r.EventStatusRecorder.Report(ctx, o, netboxv1.ConditionVlanAssignedFalse, corev1.EventTypeWarning, err)
		...
		r.EventStatusRecorder.Report(ctx, o, netboxv1.ConditionVlanAssignedTrue, corev1.EventTypeNormal, nil)
	} else {
	...
	if apismeta.IsStatusConditionTrue(vlan.Status.Conditions, "Ready") {
		...
		r.EventStatusRecorder.Report(ctx, o, netboxv1.ConditionVlanClaimReadyTrue, corev1.EventTypeNormal, nil)
	} else {
		r.EventStatusRecorder.Report(ctx, o, netboxv1.ConditionVlanClaimReadyFalse, corev1.EventTypeWarning, nil)
		return ctrl.Result{Requeue: true}, nil
	}
